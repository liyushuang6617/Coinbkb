...
public class FragmentZFAdapter extends BaseAdapter {
    ...
    private List<ZhangFuBean.dataBean> list;

    public FragmentZFAdapter(Context context, List<ZhangFuBean.dataBean> list) {
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        if (view == null) {
            ...
            view = LayoutInflater.from(context).inflate(R.layout.fragmenttwolist_item, null);
        ...
        ZhangFuBean.dataBean bean = list.get(i);
        ...
        holder.A4.setText("￥ " + bean.getCny_price());
        if (bean.getChange() != null) {
            if (bean.getChange().length() > 1) {
                String str = bean.getChange().substring(0, 1);
                double aA = 0;
                if (StringUtil.isBlank(bean.getChange())) {
                    aA = 0;
                } else {
                    aA = Double.parseDouble(bean.getChange());
                }
