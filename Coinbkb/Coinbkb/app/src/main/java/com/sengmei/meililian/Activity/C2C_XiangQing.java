...
public class C2C_XiangQing extends BaseActivity implements View.OnClickListener {
    ...
    private void XiangQingShow() {
        GetDataFromServerInterface mFromServerInterface = GetDataFromServer.getInstance(C2C_XiangQing.this).getService();
        ...
        indexdata.enqueue(new Callback<C2C_XiangQingBean>() {
            ...
            public void onResponse(Call<C2C_XiangQingBean> call, Response<C2C_XiangQingBean> response) {
                ...
                if (response.body().getType().equals("ok")) {
                    ...
                    a5.setText(response.body().getMessage().getCurrency_uci().getReal_name());
                    a6.setText(response.body().getMessage().getOther_phone());
                    a7.setText(response.body().getMessage().getCurrency_uci().getAli_pay_account());
                    a8.setText(response.body().getMessage().getCurrency_uci().getWe_chat_account_name());
                    a9.setText(response.body().getMessage().getCurrency_uci().getWe_chat_account());
                    a10.setText(response.body().getMessage().getCurrency_uci().getBank_account_name());
                    a11.setText(response.body().getMessage().getCurrency_uci().getBank_account());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.bt1:
                if (status.equals("1")){
                    Str="quxiao";
                    di=new CustomDialog(C2C_XiangQing.this,R.style.customDialog,R.layout.c2cfabudialog_item);
                    di.show();
                    dia(view);
                }
                break;
            case R.id.bt2:
                ...
                if (Typs.equals("sell")&Typs.equals("2")){

                    Str="shoukuan";
                    d2=new CustomDialog(C2C_XiangQing.this,R.style.customDialog,R.layout.c2cfabudialog_item);
                    d2.show();
                    dia(view);
                }
                break;
    ...
    public void dia(View v){
        LayoutInflater in = LayoutInflater.from(C2C_XiangQing.this);
    ...
    private void QuXiaoShow(final String ids,final String ps) {
        ...
        indexdata.enqueue(new Callback<ZhuCeBean>() {
            ...
            public void onResponse(Call<ZhuCeBean> call, Response<ZhuCeBean> response) {
                ...
                Toast.makeText(C2C_XiangQing.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                finish();
